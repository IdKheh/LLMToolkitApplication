const STRINGS = {
    POSTaggingName: "Part of Speech Tagging",
    GERLanguageToolName: "Grammar check using LanguageTool",
    GERIKorektorName: "Grammar check using IKorektor",
    GERValue: "Value is a percentage of sentences in a text that contain grammatical mistakes [Grammatical Error Rate].",
    showErrorsText: "SHOW ERRORS",
    hideErrorsText: "HIDE ERRORS",
    spellCheckerName: "Spelling Checker",
    spellCheckerValue: "Value is a number of words with spelling errors found in the given text.",
    downloadPDF: "Download report in PDF format",
    loadingPDFText: "Loading document...",
    reportNamePDF: "LLM_output_grading_report.pdf",
    PDFTitle: "LLM output grading report",
    languageDetectionName: "Language Detection",
    languageDetectionValue: "Top 3 detected languages and confidence for the given text.",
};

export default STRINGS;